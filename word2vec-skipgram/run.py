...
corpus_root = '../a1/JOURNALISM.BG/C-MassMedia'
...
if len(sys.argv)>1 and sys.argv[1] == 'cumulative':
    U,V,W = w2v_sgd.stochasticGradientDescend(data,np.copy(U0),np.copy(V0),np.copy(W0),contextFunction,grads.lossAndGradientCumulative)
else:
    U,V,W = w2v_sgd.stochasticGradientDescend(data,np.copy(U0),np.copy(V0),np.copy(W0),contextFunction,grads.lossAndGradientBatched)
